...
namespace PatchworkLauncher
{
	internal class PatchingHelper
	{
		...
		public static void RestorePatchedFile(string file)
		{
			...
			{
				throw new Exception("The existing file was found to be modified, but a backup of the original could not be found. This means the disk is in an unknown state.");
			}
		...
		{
			var assembly = AssemblyCache.Default.ReadAssembly(file);
			var history = from attr in assembly.GetCustomAttributes<PatchedByAssemblyAttribute>() orderby attr.Index select attr.ToPatchApplicationMetadata();
		...
		public static bool DoesFileMatchPatchList(string file, string targetFile, IEnumerable<PatchInstruction> patches)
		{
			...
			var patchMetas = from patch in patches select patch.Patch.PatchAssembly.GetAssemblyMetadataString();
		...
		public static AppInfoFactory LoadAppInfoFactory(string assembly)
		{
			...
			var gameInfoFactories = from type in gameInfoAssembly.GetTypes() where type.GetCustomAttribute<AppInfoFactoryAttribute>() != null select type;
			...
			{
				throw new ArgumentException($"The AppInfo assembly had more than one class decorated with {nameof(AppInfoFactoryAttribute)}");
			}
