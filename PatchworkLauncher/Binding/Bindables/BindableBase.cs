...
namespace Patchwork.Utility.Binding
{
	public abstract class BindableBase<T> : IBindable<T>
	{
		...
		{
			add { _ifaceHasChanged += value; }
			remove { _ifaceHasChanged -= value; }
		}
		...
		protected BindableBase() : this(false)
		{
		}
		...
		private BindableBase(bool alwaysValid)
		{
			...
			else
			{
				...
				RuleChanged += delegate { IsValid.NotifyChange(); };
				HasChanged += delegate { IsValid.NotifyChange(); };
				HasChanged += delegate { _ifaceHasChanged?.Invoke(this); };
			}
		}
		...
		public IBindable<bool> IsValid { get; private set; }
		...
		private class AlwaysTrueBindable : BindableBase<bool>
		{
			...
			private AlwaysTrueBindable() : base(true)
			{
				IsValid = Instance;
			}
			...
			{
				get { return true; }
				set { throw Errors.Bindable_is_readonly(); }
			}
		...
		private class ValidationBindable : BindableBase<bool>
		{
			...
			public override bool Value
			{
				get { return _bindable.Rules.All(rule => rule.Validate(_bindable.Value)); }
				set { throw Errors.Bindable_is_readonly(); }
			}
